...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ['image/jpeg','image/png','image/gif','application/pdf'].includes(file.mimetype) ? cb(null, true) : cb(new Error('Invalid file type'));
  }
});
...
router.post('/', authMiddleware, upload.single('photo'), async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Ticket created successfully', ticket: ticket.rows[0] });
  } catch (error) {
...
router.put('/:id/status', authMiddleware, isAdmin, async (req, res) => {
  try {
    ...
    const updated = await db.query('SELECT * FROM tickets WHERE id = $1', [id]);
    res.json({ success: true, message: 'Ticket status updated successfully', ticket: updated.rows[0] });
  } catch (error) {
