...
export const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [statsRes, ordersChartRes, revenueRes, marketsRes, usersRes, ticketsRes, ordersListRes] = await Promise.all([
          adminApi.getDashboardStats(),
          adminApi.getOrdersChart(),    // chart data only
          adminApi.getRevenueChart(),
          adminApi.getOrdersByMarket(),
          adminApi.listUsers(),
          adminApi.listTickets(),
          adminApi.listOrders(),        // separate call for the orders table
        ])

        setStats(statsRes.data || {})
        setChartData({
          orders: ordersChartRes.data?.data || [],
          revenue: revenueRes.data?.data || [],
          markets: marketsRes.data?.data || [],
        })
        setUsers(usersRes.data?.users || [])
        setOrders(ordersListRes.data?.orders || [])   // uses dedicated listOrders call
